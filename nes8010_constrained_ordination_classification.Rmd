...
linear (Principal Components Analysis PCA  via `ordi_pca` command) , unimodal
(Correspondence Analysis CA via `ordi_ca` command) and non-metric (Non-metric
multidimensional scaling NMDS via `ordi_nmds` command).  Typically your input 
table of data will be in the form of samples (sites) as rows and attributes 
(species) as columns, e.g.:
...
## Before you begin
In RStudio, please create a **new project** in an empty folder, then create an empty R Script file in which you should include all your commands, comments etc. Put a copy of the `nes8010.R` script (available on Canvas) into your project folder. Begin your script with a comment to explain its contents, then begin by loading various libraries and running the `nes8010.R` script:
...
Constrained unimodal ordination, Constrained Correspondence Analysis (CCA) is also performed with the `ordi_cca` command, whilst constrained linear ordination, Redundancy Analysis (RDA) is done via the `ordi_rda` command.  You indicate to R that you are doing a constrained rather than unconstrained analysis by inclusion of the explanatory variables in the command.:
...
```{r biplot spp bp, eval=TRUE}
ordi_plot(varespec.cca, display=c("species", "bp"), geom="text")
```

If you want to display the two plots side-by-side, save the plots into an R object, then use the `multi_plot()` function (from `nes8010.R`):
...
This plot with all the species in it is rather cluttered, especially for the **ubiquitous** species that occur in the centre of the plot. Therefore, you may prefer to annotate species
interactively:

```{r biplot_spp_bp, eval=FALSE}
...
On your own: repeat the CCA but __partial-out__ or __condition__ the effect of Baresoil so that its effects are accounted for already in the final results.  Use the `Condition` option in CCA; i.e. change `Baresoil` to `Condition(Baresoil)` in your CCA equation.  What is the impact of this? [__Hint:__ when you partial out a variable, the effects of the other variables is shown after taking account of the partialled-out variable]
...
This displays your sample scores and environmental variables, and a second graph of species scores and environmental variables.  As before, the A1 variable (soil depth), a continuous variable, is represented by an arrow, its length suggesting that it is an important explanatory variable in the analysis.  The Management variable, being a categorical factor, is now displayed by points rather than arrows.  Management techniques BF (biological farming) and HF (hobby farming) are similar to each other, and differ from NM (nature conservation management) along the RDA Axis 1 (x-axis).  SF (standard farming) unsurprisingly differs from all three of the others, along the second RDA Axis 1 (y-axis).

**Note** You can see that the `Management` variable name is actually quite long; in the display you get the variable name plus the "level" within the factor, i.e. `ManagementSF`, `ManagementNM` etc. These are quite long and may be awkward in your display. If this is a problem you can either:

* extend the x-axis and y-axis limits using `xlim(-2.5, 1.5)` and `ylim(-2, 2.25)` to your `ordi_plot()` call as two extra lines
* shorten the name of the variable, e.g. from `Management` to `Mgt` so that it is easier to display.
...
ordi_pca(Y)     # Linear unconstrained: Principal Components Analysis
```

Sometimes the arch effect is a problem, in which case try:

```{r examples cca metaMDS, eval=FALSE}
ordi_ca(Y)     # Unimodal unconstrained: Correspondence Analysis
ordi_nmds(Y) # Non-metric distance-based: Non-metric MDS
```
...
ordi_rda(Y ~ x1 + x2 + x3, data=X)   # Linear: Redundancy Analysis
ordi_cca(Y ~ x1 + x2 + x3, data=X)   # Unimodal: Constrained CA 
```

To display:

```{r example plot types, eval=FALSE}
ordi_plot(Y.rda)  # Triplot Samples, Attributes, explanatory variables
ordi_plot(Y.rda, display=c("sites", "bp"))   #Biplot=Samples, Arrows 
ordi_plot(Y.rda, display=c("sites", "cn"))   #Biplot=Samples, Centroids 
ordi_plot(Y.rda, display=c("species", "cn")) #Biplot=Attribut,Centroids
```
...
```
Next, you'll find it easiest to extract the dune PC1 and PC2 scores using `ordi_scores()`, and add the management to them via the `mutate()` function. Finally you can assemble them into a `ggplot` graph. The `nes8010.R` script adds a `geom_chull()` function which makes it easier to add a "convex hull" around a set of points in your graph:

```{r convex_hulls-setup}
data(dune)
data(dune.env)
dune.pca <- ordi_pca(dune)
```
...
Sometimes you may want to change the default names in a legend. For example in this dataset, `Haypastu` is used instead of `Hay pasture`. Likewise, you might not want `Use` but would prefer `Field use` in the legend. You can tweak these with:
...
### Aims
These exercises will allow you to explore more of the theory that is covered my lectures of the module, in particular classification of multivariate data (and its display), and ways of overlaying classification results onto ordinations to aid their interpretation.

## Multivariate classification or clustering
The `vegan` package has a number of functions that allow you to do hierarchical classifications of species x samples data tables, and to sort your original tables of data to show the main trends.  Packages such as `MASS` (already installed by default) and `labdsv` also provide functions for fuzzy clustering and indicator species analysis.  We will focus on the tools available in `vegan`, using its example `dune` dataset.
...
**Question** What happens to your display if in the `mutate` code line you omit
the `as.factor()` statement and simply put `group_code = dune.grp` ? Why does
this change occur?

Well done - the above graph shows a multivariate ordination (via PCA) of your dune data, overlaid with a multivariate classification (via hclust) of the same data.  You also know that the soil depth differs between the three classes, with the highest values (and greatest variability) in Group 2.


## Species classification
Just as you can classify the sites, so you can classify species; you simply have to transpose (turn through 90 degrees) the original dune data.frame via the `t()` function when calling the `vegdist` command. The Raup-Crick distance measure is particularly useful for species, especially when there are many zeros in your table, which is typical of ecological data.
...
This table re-orders your sites and species in a compact form, based on the results of your PCA of the dune data. The abundance is shown by a simple score of 1 to 9, with a dot representing absence.

A particularly nice trick is to use `tabasco()` and then cross-display the relevant dendrograms for the sites and species at either side. Let's do this with the dune data:

```{r tabasco, eval=FALSE}
...
**Note** After running the last command when your heatmap is displayed most species will not be showing. Click on the Plots window (top-right) to enlarge the window to full-size to see all the species.
