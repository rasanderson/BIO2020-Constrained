...
commands below we ask `autoplot()` to `layers=c("sites", "biplot")` or `layers=c("species", "biplot")` where `biplot` represents the "biplot" arrow
...
autoplot(varespec_cca, geom="text", layers=c("sites", "biplot"), legend.position = "none")
autoplot(varespec_cca, geom="text", layers=c("species", "biplot"), legend.position = "none")
```
...
autoplot(varespec_cca2, geom="text", layers=c("sites", "biplot"), legend.position = "none")
autoplot(varespec_cca2, geom="text", layers=c("species", "biplot"), legend.position = "none")
```
...
is called via the `rda()` function, and R recognises that it should be RDA rather
than PCA as soon as you provide explanatory variables. 
...
* Pipe your `autoplot` via the `%>%` symbol into `gf_lims()` to extend the x-axis
...
autoplot(dune_rda, geom="text", layers=c("sites", "centroids", "biplot"), legend.position = "none")

# Plot species and explanatories. Add "arrows=FALSE" to reduce clutter
autoplot(dune_rda, geom="text", layers=c("species", "centroids", "biplot"), legend.position = "none", arrows=FALSE)
```
```{r dune_rda-solution}
# Undertake the RDA with just two explanatory variables
dune_rda <- rda(dune ~ A1 + Management, data=dune.env)
...
# Plots
autoplot(dune_rda2, geom="text", layers=c("sites", "centroids", "biplot"), legend.position = "none")
autoplot(dune_rda2, geom="text", layers=c("species", "centroids", "biplot"), legend.position = "none", arrows=FALSE)
...
these are shown with points rather than arrows
...
caution and retain some non-significant explanatory variables.
