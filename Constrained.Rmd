...
attributes (species, metagenomics data etc.). They provide a useful method to 
...
QQ plots) via the `mosaic` package `mplot()` function. Don't worry if you are
not fully familiar with linear models and diagnostic plots yet, as these will be
covered in more detail elsewhere in NES8010. It is simply one way of trying to
measure the effect of an environmental variable (soil moisture) on the species,
and diagnostic plots check whether the linear model is working correctly.
...
was the most common method of analysis. It is sometimes referred to as **indirect gradient analysis** and was widely used until the 1990's. However, it has problems...
...
* Bird survey in one woodland should be independent from birds in another wood
* Fish survey data in the North Sea should be independent from Irish Sea data
...
* Modify `ordi_plot` by adding the `+` symbol at the end then use the `ggplot2`
function `xlim`
...
There are lots of potential risks of unthinking use of stepwise selection, as it
is easy to discard biologically meaningful variables. Therefore, if you look at
the final output from your permutation ANOVA table, you will notice that several
"marginally non-significant" variables have been retained. However, remember
that the p<0.05 significance threshold is an **arbitrary convention** and so it
is useful to also check borderline variables too.
